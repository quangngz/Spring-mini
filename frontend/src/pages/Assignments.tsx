...
const Assignments = () => {
  ...
  // Submission handled via reusable component
      ...
      try {
        ...
        await Promise.all(
          (coursesData || []).map(async (course) => {
            try {
              const courseAssignments = await assignmentsApi.getAll(course.id);
  ...
  // Removed local submit handler; now using SubmissionButton component
  ...
  const allAssignments = Object.entries(assignments).flatMap(([courseCode, courseAssignments]) =>
    courseAssignments.map(assignment => ({
      ...assignment,
      courseCode,
      courseId: courses.find(c => c.courseCode === courseCode)?.id,
                ...
                >
                  ...
                  <CardContent>
                    <div className="flex items-center justify-between">
                      ...
                      <SubmissionButton assignment={assignment} courseId={assignment.courseId!} />
                    </div>
                  </CardContent>
                </Card>
              );
